...
## ✅ 기본 람다 표현식 예제
```java
Runnable greet = () -> System.out.println("Hello, Java Lambda!");
greet.run(); // 출력: Hello, Java Lambda!
```

- Runnable은 run()이라는 단일 메서드를 가진 함수형 인터페이스
- () -> 실행문 형태로 람다 정의
...
## ✅ 고차 함수처럼 람다 전달
```java
public static void hello(Runnable process) {
    System.out.println("프로그램 시작");
    process.run();
    System.out.println("프로그램 종료");
}
...
## ✅ 캡처 변수 사용 예제
```java
public static Supplier<String> makePrinter(String prefix) {
    return () -> prefix + ": " + System.currentTimeMillis();
}

Supplier<String> printer = makePrinter("현재 시간");
System.out.println(printer.get()); // 예: 현재 시간: 1708200000000
```

- 람다 내부에서 외부 변수 prefix를 캡처
- 함수 종료 후에도 유지됨

## ✅ 전략 패턴처럼 람다 전달
```java
public static void executeStrategy(Function<Integer, String> strategy) {
    System.out.println(strategy.apply(10));
}

executeStrategy(value -> value % 2 == 0 ? "짝수" : "홀수"); // 출력: 짝수
```

- Function<Integer, String> 타입의 람다를 전략처럼 전달
- 조건에 따라 결과 반환
...
- Java 8부터 @FunctionalInterface 어노테이션으로 명시 가능 (선택 사항)
- 람다 표현식은 이 추상 메서드를 구현하는 익명 함수 객체를 생성함
### 예시:
```java
@FunctionalInterface
interface Hello {
...
이 인터페이스를 기반으로 람다를 정의할 수 있음:
```java
Hello hello = () -> System.out.println("Hello from lambda!");
hello.say(); // 출력: Hello from lambda!
```
## 📦 자바에서 자주 쓰는 함수형 인터페이스
| 인터페이스       | 메서드 시그니처         | 설명                              |
|------------------|--------------------------|-----------------------------------|
| `Runnable`       | `void run()`             | 인자 없음, 반환 없음. 주로 쓰레드 실행용 |
| `Supplier<T>`    | `T get()`                | 인자 없음, 반환 있음. 값 공급자 역할     |
| `Consumer<T>`    | `void accept(T t)`       | 인자 있음, 반환 없음. 값 소비자 역할     |
| `Function<T,R>`  | `R apply(T t)`           | 인자 있음, 반환 있음. 값 변환에 사용     |
| `Predicate<T>`   | `boolean test(T t)`      | 인자 있음, boolean 반환. 조건 검사용     |



## ✨ 핵심 요약
- Java 람다는 함수형 인터페이스 기반
- 람다 표현식은 해당 인터페이스의 단일 추상 메서드를 구현
- 클래스 없이도 코드 조각을 값처럼 전달 가능
