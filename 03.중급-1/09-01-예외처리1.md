...
# 예외 처리1 - 이론
사용자 입력에 따라 오류를 유발하고, 그 오류가 제대로 처리되지 않았을 때 발생하는 문제를 확인할 수 있습니다.
...
## 📦 흐름 구조 (Mermaid)
flowchart TD
    A[MainV1 입력] --> B[NetworkServiceV1_2]
    B --> C[client.connect()]
    C --> D{connect 성공?}
    D -- 아니오 --> E[오류 로그 출력 후 return]
    D -- 예 --> F[client.send(data)]
    F --> G{send 성공?}
    G -- 아니오 --> H[오류 로그 출력 후 return]
    G -- 예 --> I[client.disconnect()]
    H --> I
    E --> I
