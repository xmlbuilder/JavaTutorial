...
```mermaid
flowchart LR
    A[String 문자] --> B[OutputStreamWriter]
    B --> C[byte array 인코딩]
...
![Reader Writer](/image/writer_reader.png)
...
sequenceDiagram
    ...
    App->>BufferedWriter: write("ABC 가나다")
...
| 타입         | `abstract class`                                            |
| 상속 구조    | `Object` → `Reader`                                         |
| 주요 메서드  | `read()`, `read(char[] cbuf)`, `close()`                    |
| 하위 클래스  | `BufferedReader`, `InputStreamReader`, `FileReader`, `StringReader` |
...
| 하위 클래스  | `BufferedWriter`, `OutputStreamWriter`, `FileWriter`, `StringWriter` |


## 🔄 공통점 요약
- 둘 다 java.io 패키지에 포함
- 추상 클래스이며, 하위 클래스에서 구체적인 동작을 구현
- 스트림 기반으로 문자 데이터를 처리
- close() 메서드를 통해 리소스 해제 필요

---
# 바이트 기반 / 문자 기반 연결

Java의 I/O 클래스는 **바이트 기반(Stream 계열)** 과 **문자 기반(Reader/Writer 계열)** 로 명확히 나뉘기 때문에,  
각 계열을 따로 설명하는 게 훨씬 이해하기 쉽고 정확합니다.

## 🔹 바이트 기반: InputStream / OutputStream
| 항목         | 내용                                                                 |
|--------------|----------------------------------------------------------------------|
| 계열         | 바이트 기반                                                          |
| 주요 클래스  | `InputStream`, `OutputStream`                                        |
| 하위 클래스  | `FileInputStream`, `BufferedInputStream`, `ByteArrayInputStream`<br>`FileOutputStream`, `BufferedOutputStream`, `ByteArrayOutputStream` |
| 주요 메서드  | `read()`, `read(byte[])`, `write(int)`, `write(byte[])`              |
...
|--------------|----------------------------------------------------------------------|
| 계열         | 문자 기반                                                            |
| 주요 클래스  | `Reader`, `Writer`                                                   |
| 하위 클래스  | `BufferedReader`, `InputStreamReader`, `FileReader`, `StringReader`<br>`BufferedWriter`, `OutputStreamWriter`, `FileWriter`, `StringWriter` |
| 주요 메서드  | `read()`, `read(char[])`, `write(String)`, `write(char[])`           |


## 🔄 연결 고리: 바이트 ↔ 문자 변환
| 클래스              | 연결 관계 설명                                 |
|---------------------|--------------------------------------------------|
| `**InputStreamReader**`   | `InputStream` → `Reader` (바이트 → 문자 변환)   |
| `**OutputStreamWriter**`  | `Writer` → `OutputStream` (문자 → 바이트 변환)  |

- 이 두 클래스는 Java I/O에서 문자 인코딩 처리의 핵심 역할을 하며, 바이트 스트림과 문자 스트림 간의 다리 역할을 합니다
```java
